                        ...
                        <ul class="bleft__list">
                            {foreach from=$news item=item key=key name=name}
                                <li class="bleft__item post">
                                    <img src="uploads/{$item.img}" alt="" class="bleft__img">
                                    <div class="post__info-wrapper">
                                        <div class="post__date">
                                            ...
                                            <time class="post__time"
                                                datetime="2022-11-30">{$item.date|date_format:"%d.%m.%Y г"}</time>
                                        </div>
                                        <div class="post__views">
                                            ...
                                            <span class="post__count">{$item.views}</span>
                                        </div>
                                    </div>
                                    <h3 class="post__header">{$item.header}</h3>
                                    <p class="post__descr">
                                        {$item.text}
                                    </p>
                                    <a href="read-news?id={$item.id}" class="post__link post-link">
                        ...
                        <div class="bleft__pages pages">
                            <div class="pages__left">
                                ...
                                {if $page > 1}
                                    ...
                                    <a href="{$page_url}" class="pages__span">Первая страница</span>
                                    {/if}
                                ...
                                <ul class="pages__list">
                                    {for $x = $initial_num; $x < $condition_limit_num; $x++}
                                        {if ($x > 0) && ($x <= $total_pages)}
                                            {if $x == $page}
                                                <li class="pages__item"><a class="pages__item--active" href='#'>{$x}</a></li>
                                            {else}
                                                <li class="pages__item"><a href='{$page_url}page={$x}'>{$x}</a></li>
                                            {/if}
                                        {/if}
                                    {/for}
                                </ul>
                            ...
                            <div class="pages__right">
                                {if $page < $total_pages}
                                    <a href="{$page_url}page={$total_pages}" class="pages__span-right">Следующая
                                        страница</a>
                    ...
                    <div class="blog__right">
                        {* <div class="blog__search search">
                            <form action="" class="form search__form">
                                <input type="text" class="search__input" placeholder="Поиск" name="" id="">
                        ...
                        <div class="posts__wrapper recent-posts">
                            ...
                            <ul class="recent-posts__list recpost">
                                {foreach from=$recentNews item=item key=key name=name}
                                    <li class="recent-posts__item">
                                        
                                        <a href="read-news?id={$item.id}" class="recpost__wrapper">
                                            <div class="recpost__left">
                                                <img src="uploads/{$item.img}" alt="" class="recpost__img">
                                            </div>
                                            <div class="recpost__right">
                                                <time class="recpost__time" datetime="YYYY-MM-DD"> {$item.date|date_format: "%d.%m.%Y г"}
                                                    </time>
